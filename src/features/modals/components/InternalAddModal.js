...
import InternalAddModalField from "./InternalAddModalField";

const InternalAddModal = (props) => {
  ...
  const { mutate: postData, isLoading: mutateLoading } = useMutation({
    mutationFn: (data) =>
      API.postData(data, "http://localhost:1337/api/internal-errors"),
        ...
        <div className="grid">
          {fields.map((options, index) => {
            return (
              ...
              >
                <InternalAddModalField {...options} control={control} />
              </div>
            );
          })}
        </div>
...
export default InternalAddModal;
