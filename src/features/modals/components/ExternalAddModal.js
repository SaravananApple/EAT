...
import ExternalAddModalField from "./ExternalAddModalField";

const ExternalAddModal = (props) => {
  ...
  const { control, handleSubmit, reset, clearErrors, getValues } = useForm({
  ...
  const { mutate: postData, isLoading: mutateLoading } = useMutation({
    mutationFn: (data) =>
      API.postData(data, "http://localhost:1337/api/external-errors"),
        ...
        <div className="grid">
          {fields.map((options, index) => {
            return (
              ...
              >
                <ExternalAddModalField {...options} control={control} />
              </div>
            );
          })}
        </div>
...
export default ExternalAddModal;
