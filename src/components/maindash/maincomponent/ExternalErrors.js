...
import { useFetchExternalErrorList } from "../../../service/TaskListsService";
import ExternalErrorModal from "../../../features/modals/ExternalErrorModal";
import ExternalErrorEditField from "../../../features/modals/components/ExternalErrorEditField";
...
const ExternalErrors = () => {
  ...
  const { data: taskData, isLoading } = useFetchExternalErrorList();
  ...
  const { mutate: deleteData } = useMutation({
    mutationFn: (data) =>
      API.deleteData(data, "http://localhost:1337/api/external-errors"),
  ...
  return (
    <div>
      <Toast ref={toast} />
      <Header
        pageTitle="External Error Tracking"
        actionItems={
          <Button
            severity="warning"
            label="Add Errors"
      ...
      <div className=" surface-card shadow-2 border-round flex-grow-1 task">
        ...
        >
          ...
          <Column
            field="containment"
            header={columnsInfo["containment"].displayName}
          />
          <Column
            field="determine_root_cause"
            header={columnsInfo["determine_root_cause"].displayName}
          />
          <Column
            field="corrective_action"
            header={columnsInfo["corrective_action"].displayName}
          />
          <Column
            field="preventive_action"
            header={columnsInfo["preventive_action"].displayName}
          />
          ...
          <Column
            field="time_taken"
            body={pickTimeBodyFilter}
            header={columnsInfo["time_taken"].displayName}
            sortable
            filter
          />
